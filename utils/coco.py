...
class COCO(data.Dataset):
  def __init__(self, root, split='train', img_size=256):
    ...
    self.split = split
    self.root=root
    self.img_dir = os.path.join(self.root, 'images','%s2017' % split)
    if split == 'test':
      self.annot_path = os.path.join(self.root, 'annotations', 'image_info_test-dev2017.json')
    else:
      self.annot_path = os.path.join(self.root, 'annotations', 'instances_%s2017.json' % split)
  ...
  def __getitem__(self, index):
    ...
    # cv2.imwrite('image_{}.png'.format(img_id),img)
    # cv2.imshow('img', img)
    # cv2.waitKey()
    # -----------------------------------debug---------------------------------
...
class COCO_eval(COCO):
  def __init__(self, root, split, test_scales=(1,), test_flip=False, fix_size=False):
    super(COCO_eval, self).__init__(root, split)
...
if __name__=='__main__':
  ...
  for e,batch in enumerate(dataloader):
    print(batch['image'].shape) # [N x 3 x w x h]
    print(batch['hmap'].shape) # [N x C x w/R x h/R]
    print(batch['w_h_'].shape) # [N x max_obj x 2]
    print(batch['regs'].shape) # [N x max_obj x 2]
    print(batch['inds'].shape) # [N x max_obj]
    print(batch['inds'])
    print(batch['ind_masks'].shape) # [N x max_obj]
    if e>1:
      break