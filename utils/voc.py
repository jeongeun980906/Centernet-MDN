...
class PascalVOC(data.Dataset):
  def __init__(self, root, split='train', img_size=384, **kwargs):
...
class PascalVOC_eval(PascalVOC):
  def __init__(self, root, split, test_scales=(1,), test_flip=False, fix_size=True, **kwargs):
    super(PascalVOC_eval, self).__init__(root, split, **kwargs)
  ...
  def run_eval(self, results, save_dir=None):
    ...
    eval_map = eval_mAP(os.path.join(self.root, 'VOCdevkit'))
